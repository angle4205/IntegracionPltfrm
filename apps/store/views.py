...
def obtener_carrito(request):
    try:
        cart = get_or_create_cart(request)
        cart.calcular_totales()  # Asegura que los totales estén actualizados
        ...
        return JsonResponse(
            {
                ...
                "iva": int(cart.iva),
                "costo_despacho": int(cart.costo_despacho),
                "total": int(cart.total),
                "ajuste_stripe": (
                    cart.ajuste_stripe() if hasattr(cart, "ajuste_stripe") else 0
                ),
                "total_para_stripe": cart.total_para_stripe(),
                "ajuste_stripe": cart.ajuste_stripe(),
